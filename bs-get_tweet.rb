...
$url='https://twitter.com/BernieSanders?ref_src=twsrc%5Egoogle%7Ctwcamp%5Eserp%7Ctwgr%5Eauthor'
browser.goto($url)
...
$handle=browser.element(class: 'ProfileHeaderCard-nameLink').text.strip
	...
	#store name in file
	File.open("bs/handle.txt", "w") do |f|     
	f.write($handle)   
	end
...
#store name in file
filename="bs/dt_tweet"+z.to_s+".txt"
